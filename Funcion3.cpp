...
void listarProductosMasVendidos() {
    ...
    while (getline(archivo, linea)) {
        ...
        stringstream ss(linea);
        string idUsuario, nombreUsuario, idProdStr, nombreProd, cantidadStr, precio, subtotal;
...
void generarOrdenDeCompra(const Usuario &usuario, vector<ItemCarrito> &carrito, int &contadorOrdenes) {
    ...
    OrdenDeCompra orden;
    orden.idOrden = ++contadorOrdenes; // Aumenta el ID de la orden autom?ticamente
    ...
    // Calcular impuestos (ejemplo: 19% de IVA) y env?o (ejemplo: 5000 fijos)
    ...
    // Vaciar el carrito despu?s de realizar la compra
    carrito.clear(); 
}
//punto 2
int main() {
